    ...
    global.error_msg = "starting loop"
    {
        ...
        for(i = 1; i <= instance_number(obj_player_current); i++)
        {
            ...
            //if global.player_array[i].state = PLAYER_STATE.is_alive // if player is alive
            {
                ...
                //if ai_vision(temp2,300,image_angle,350) // if the player is seen
                {seen = true; global.error_msg = "AI seen"}
                else
                {
                    global.error_msg = "AI not seen"
                    seen = false
                    hspeed = 0
                    vspeed = 0
                }
